...
object OrbitTermMigrations {
    ...
    /**
     * Give every already-persisted intent one durable replay identity. The
     * value is generated once during migration and is never regenerated by a
     * retry, process restart, or connectivity transition.
     */
    val V8_TO_V9 = object : Migration(8, 9) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE asset_sync_outbox ADD COLUMN operationId TEXT NOT NULL DEFAULT ''")
            db.execSQL(
                "UPDATE asset_sync_outbox SET operationId = " +
                    "lower(hex(randomblob(4))) || '-' || lower(hex(randomblob(2))) || '-' || " +
                    "lower(hex(randomblob(2))) || '-' || lower(hex(randomblob(2))) || '-' || " +
                    "lower(hex(randomblob(6))) WHERE operationId = ''",
            )
        }
    }

    /**
     * Persist a privacy-safe delivery state so permanent failures remain
     * recoverable without being selected by every background sync pass.
     */
    val V9_TO_V10 = object : Migration(9, 10) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE asset_sync_outbox ADD COLUMN deliveryDisposition TEXT NOT NULL DEFAULT 'READY'")
            db.execSQL("ALTER TABLE asset_sync_outbox ADD COLUMN failureCode TEXT")
            db.execSQL("ALTER TABLE asset_sync_outbox ADD COLUMN attemptCount INTEGER NOT NULL DEFAULT 0")
        }
    }

    /** Persist server Retry-After across process death, lock, and reboot. */
